...
class LayoutOrchestrator(private val context: Context) {
    ...
    suspend fun applyLayout(config: LayoutConfig): LayoutResult {
        ...
        try {
            // Enable freeform flags if not already set
            ...
            // Enhanced launch sequence for better reliability
            val results = mutableListOf<LaunchResult>()
           
            // Launch bottom apps first to prevent split-screen takeover (critical for Lenovo)
            ...
            // Position and resize with verification
            ...
            return LayoutResult(
                success = successCount >= (totalOperations * 0.7), // 70% success rate threshold
                message = message,
                results = results
            )
        } catch (e: Exception) {
    ...
    private suspend fun positionAndResizeApp(
        ...
        return try {
            ...
            delay(300)
            // Verification step could be added here
            true
        } catch (e: Exception) {
